...
import { CREATE_AGENCY_CONTRACT } from '@/components/widgets/mip-agency-contract/mip-agency-contract.gql';
import {
	MIPAgencyContractSchema,
	validationSchema,
	initialValues,
} from '@/components/widgets/mip-agency-contract/mip-agency-contract.validation';
...
export const NewMIPAgencyContract = () => {
	...
	const formik = useFormik<MIPAgencyContractSchema>({
		...
		validationSchema,
		onSubmit: (data) => {
			const { agency_contract_properties, mip_agency_contract_properties } =
				data;
			createAgencyContract({
				variables: {
					input: normalizePayload({
						agency_contract_properties: {
							agency_contract_type: AgencyContractType.MipAgencyContract,
							...agency_contract_properties,
						},
	...
	const onDefaultCommissionValues = () => {
		formik.setFieldValue(
			'mip_agency_contract_properties.agency_contract_commission.percent',
			4,
		);
		formik.setFieldValue(
			'mip_agency_contract_properties.agency_contract_commission.threshold',
			0,
		);
		formik.setFieldValue(
			'mip_agency_contract_properties.agency_contract_commission.max_days',
			5,
		);
	};
					...
					<Container>
						<Form>
							...
							<AgencyContractCommission
								prefix='mip_agency_contract_properties'
								values={
									formik.values.mip_agency_contract_properties
										.agency_contract_commission
								}
								touched={
									formik.touched.mip_agency_contract_properties
										?.agency_contract_commission
								}
								errors={
									formik.errors.mip_agency_contract_properties
										?.agency_contract_commission
								}
